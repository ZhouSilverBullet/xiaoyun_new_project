...
public class SiriView extends View {
    ...
    private int computeSpringValue1;
    private int computeSpringValue2;
    ...
    public SiriView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = getResources().obtainAttributes(attrs, R.styleable.SiriView);
        try {
            computeSpringValue1 = a.getInt(R.styleable.SiriView_sv_base_value, 30);
            computeSpringValue2 = a.getInt(R.styleable.SiriView_sv_wave_value, 70);
        } finally {
            a.recycle();
        }
...
//        if (isSpeaking) {
//            computeSpringValue1 = 30;
//            computeSpringValue2 = 70;
//        } else {
//            computeSpringValue1 = 10;
//            computeSpringValue2 = 10;
//        }
//
//        //计算弹起的点和百分比
//        computeSpring(rowOnePoints);
    ...
    private void computeSpring(AJPoint[] rowPoints) {
        for (int i = 1; i < rowPoints.length - 1; i++) {
            ...
            if (!rowPoints[i - 1].needSpring) {//前面一个是没有弹起的
                ...
                rowPoints[i].springFactor = (computeSpringValue1 + random.nextInt(computeSpringValue2)) / 100F;
            }
        }
    }
